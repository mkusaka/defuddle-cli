...
program
  ...
  .option(
    "-p, --property <name>",
    "Extract a specific property (e.g., title, description, domain)",
  )
  ...
  .action(async (source: string, options: ParseOptions) => {
    try {
      ...
      try {
        ...
        try {
          ...
          if (options.property) {
            ...
            if (property in result) {
              output =
                result[property as keyof typeof result]?.toString() || "";
            } else {
              console.error(
                chalk.red(
                  `Error: Property "${property}" not found in response`,
                ),
              );
              process.exit(1);
            }
        ...
        console.error(
          chalk.red("Error loading content:"),
...
program
  ...
  .description(
    "Extract content from URLs in a sitemap and save as markdown files with YAML frontmatter",
  )
  .argument("<url>", "Sitemap URL to fetch URLs from")
  ...
  .action(
    async (url: string, outputDir: string, options: SitemapExtractOptions) => {
      try {
        if (options.debug) {
          console.log(chalk.blue("Debug mode enabled"));
          console.log(chalk.blue(`Fetching sitemap from: ${url}`));
        ...
        try {
          ...
          console.log(
            chalk.blue(
              `Rate limit set to ${rateLimitPerSecond} requests per second${rateLimitPerSecond === 0 ? " (disabled)" : ""}`,
            ),
          );
          ...
          const throttle =
            rateLimitPerSecond > 0
              ...
              : (fn: Function) => fn; // 0の場合はスロットルを適用せずに関数をそのまま実行
