...
program
	...
	.option('-l, --rate-limit <number>', 'Maximum number of requests per second (0 to disable)', '1')
	.action(async (url: string, outputDir: string, options: SitemapExtractOptions) => {
		try {
			...
			try {
				...
				console.log(chalk.blue(`Rate limit set to ${rateLimitPerSecond} requests per second${rateLimitPerSecond === 0 ? ' (disabled)' : ''}`));
				
				const throttle = rateLimitPerSecond > 0 
					? pThrottle({
